...
async def main() -> None:
    if not (os.environ.get("OPENAI_API_KEY") or os.environ.get("GROQ_API_KEY")):
        print(
            "No API key found. Add OPENAI_API_KEY (to use OpenAI) or "
            "GROQ_API_KEY (to use Groq) to a .env file (see .env.example) "
    ...
    if len(sys.argv) > 1:
        ...
        # Uses a fresh, isolated session rather than the persistent
        # employee_session — each single-shot invocation is meant to be
        # an independent question, not a continuation of whatever was
        # asked (and possibly answered wrong) in a previous run. The
        # persistent session below is only for the interactive loop,
        # where continuing one ongoing conversation is actually wanted.
        one_shot_session = SQLiteSession(f"one_shot_{uuid.uuid4().hex[:8]}", SESSION_DB_PATH)
        await run_query(" ".join(sys.argv[1:]), one_shot_session)
        return
